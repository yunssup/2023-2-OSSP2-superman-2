...
function Compare() {
  ...
  const handleAddressInputComplete = (houseNum) => {
    ...
    axios
      .post("http://52.78.118.198:8080/api/compare", postData)
      .then((response) => {
        ...
        axios
          .get(
            `http://52.78.118.198:8080/api/compare/transport/?user=${userSessionData}&house=${houseNum}`,
            {
              withCredentials: true,
            }
          )
          .then((getResponse) => {
            ...
            const houseInfoUrl = `http://52.78.118.198:8080/api/compare/houseinfo?address=${encodeURIComponent(
